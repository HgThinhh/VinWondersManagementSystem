...
{
    public partial class GUI_CapNhatSanPham : Form
    {
        private string maSanPham;
        BUS_SanPham busSanPham = new BUS_SanPham();
        BUS_CuaHang busCuaHang = new BUS_CuaHang();

        public GUI_CapNhatSanPham(ET_SanPham sp)
        {
            InitializeComponent();
            LoadCboCuaHang();
            LoadCboLoaiSanPham();
            ...
            txtTenCuaHang.Text = sp.TenSanPham;
            txtGiaBan.Text = sp.GiaBan.ToString();

            // Chọn cửa hàng tương ứng
            cboCuaHang.SelectedValue = sp.MaCuaHang;

            // Chọn loại sản phẩm tương ứng
            for (int i = 0; i < cboLoaiSanPham.Items.Count; i++)
            {
                if (cboLoaiSanPham.Items[i].ToString() == sp.LoaiSanPham)
                {
                    cboLoaiSanPham.SelectedIndex = i;
                    break;
                }
            }

            // Chọn trạng thái bán tương ứng
            for (int i = 0; i < cboTrangThaiBan.Items.Count; i++)
            {
                if (cboTrangThaiBan.Items[i].ToString() == sp.TrangThaiBan)
                {
                    cboTrangThaiBan.SelectedIndex = i;
                    break;
                }
            }
        }
        ...
        private void btnCapNhat_Click(object sender, EventArgs e)
        {
            ...
            string tenSanPham = txtTenCuaHang.Text.Trim();
            ...
            DialogResult dialogResult = MessageBox.Show(
                $"Bạn có chắc chắn muốn cập nhật sản phẩm \"{tenSanPham}\" không?",
                "Xác nhận cập nhật",
            ...
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    bool isSuccess = busSanPham.SuaSanPham(etSanPham);
                    if (isSuccess)
                    {
                    ...
                    {
                        MessageBox.Show("Cập nhật sản phẩm thất bại. Vui lòng thử lại!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                catch (Exception ex)
                {
        ...
        {
            string tenSP = txtTenCuaHang.Text.Trim();
